...
public class AxionType {
  ...
  /**
   * Returns a type that represents an unknown type that extends {@code bound}. For example, if {@code bound} is {@code
   * CharSequence.class}, this returns {@code ? extends CharSequence}. If {@code bound} is {@code Object.class}, this
   * returns {@code ?}, which is shorthand for {@code ? extends Object}.
   */
  ...
  public static Class<?> getRawType(Type type) {
    if (type instanceof Class<?>) {
      ...
      AxionContract.assertArgument(rawType instanceof Class, rawType + " not instance of Class");
  ...
  /**
   * Returns the generic supertype for {@code supertype}. For example, given a class {@code IntegerSet}, the result for
   * when supertype is {@code Set.class} is {@code Set<Integer>} and the result when the supertype is {@code
  ...
  static Type getSupertype(
    ...
    AxionContract.assertArgument(
        supertype.isAssignableFrom(contextRawType),
        supertype + " is not assignable from " + contextRawType
    );
    return resolve(context, contextRawType,
        AxionType.getGenericSupertype(context, contextRawType, supertype));
  }
  ...
  private static final class ParameterizedTypeImpl implements
    ...
    public ParameterizedTypeImpl(Type ownerType, Type rawType,
      ...
      if (rawType instanceof Class<?>) {
        ...
        AxionContract.assertArgument(ownerType != null
                || rawTypeAsClass.getEnclosingClass() == null,
            "if owner type is null, enclosing class must be null");
        AxionContract.assertArgument(ownerType == null
                || rawTypeAsClass.getEnclosingClass() != null,
            "if owner type is not null, enclosing class can't be null");
      }
      ...
      for (int t = 0; t < this.typeArguments.length; t++) {
        AxionContract.assertNotNull(this.typeArguments[t], "this.typeArguments[t] can't be null");
        AxionContract.checkNotPrimitive(this.typeArguments[t], "this.typeArguments[t] can't be primitive");
    ...
    public boolean equals(Object other) {
      return other instanceof ParameterizedType
          && AxionType.equals(this, (ParameterizedType) other);
    }
  ...
  private static final class GenericArrayTypeImpl implements GenericArrayType,
    ...
    public boolean equals(Object o) {
      return o instanceof GenericArrayType
          && AxionType.equals(this, (GenericArrayType) o);
    }
  ...
  /**
   * The WildcardType interface supports multiple upper bounds and multiple lower bounds. We only support what the Java
   * 6 language needs - at most one bound. If a lower bound is set, the upper bound must be Object.class.
   */
  private static final class WildcardTypeImpl implements WildcardType,
    ...
    public WildcardTypeImpl(Type[] upperBounds, Type[] lowerBounds) {
      AxionContract.assertArgument(lowerBounds.length <= 1, "required: lowerBounds.length <= 1");
      AxionContract.assertArgument(upperBounds.length == 1, "required: upperBounds.length == 1");

      if (lowerBounds.length == 1) {
        AxionContract.assertNotNull(lowerBounds[0], "lowerBounds[0] can't be null");
        AxionContract.checkNotPrimitive(lowerBounds[0], "lowerBounds[0] can't be primitive");
        AxionContract.assertArgument(upperBounds[0] == Object.class, "upperBounds[0] must be Object.class");
        this.lowerBound = canonicalize(lowerBounds[0]);
        this.upperBound = Object.class;

      } else {
        AxionContract.assertNotNull(upperBounds[0], "upperBounds[0] can't be null");
        AxionContract.checkNotPrimitive(upperBounds[0], "upperBounds[0] can't be primitive");
    ...
    public boolean equals(Object other) {
      return other instanceof WildcardType
          && AxionType.equals(this, (WildcardType) other);
    }
